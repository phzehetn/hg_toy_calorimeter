...
class CaloV3Generator(CaloGenerator):
    def __init__(self, calo_min_kinetic_energy, world_min_kinetic_energy, calo_max_time, world_max_time,
                 sensor_thickness=None):
        super().__init__()

        # No real reason for this. It is just to make it clear that the units are in ns and MeV if someone is reading
        # the code. This is the same as how it is defined in SystemOfUnits.h in Geant4.
        self.ns = 1
        self.MeV = 1

        if calo_min_kinetic_energy is not None:
            self.detector['calo_min_kinetic_energy'] = calo_min_kinetic_energy * self.MeV
        if world_min_kinetic_energy is not None:
            self.detector['world_min_kinetic_energy'] = world_min_kinetic_energy * self.MeV
        if calo_max_time is not None:
            self.detector['calo_max_time'] = calo_max_time * self.ns
        if world_max_time is not None:
            self.detector['world_max_time'] = world_max_time * self.ns

        if sensor_thickness is None:
            sensor_thickness = 0.0002
    ...
    def generate(self):
        ...
        for i in range(self.num_ee_layers):
            ...
            thickness = self.sensor_thickness #0.0002
            gap = 0.00001
            num_eta_cuts = l_num_eta_cuts(num_active_layer)
            num_phi_cuts = l_num_phi_cuts(num_active_layer)
            num_sensors += num_eta_cuts * num_phi_cuts
...
class CaloV4Generator(CaloGenerator):
    def __init__(self, calo_min_kinetic_energy=None, world_min_kinetic_energy=None, calo_max_time=None,
                 world_max_time=None, sensor_thickness=None):
        super().__init__()
...
# CaloV3Generator(None, None, None, None).generate()
# CaloV4Generator(None, None, None, None).generate()