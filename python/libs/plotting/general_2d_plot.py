...
class General2dBinningPlot():
    def __init__(self, bins, x_label='x-axis', y_label='y-axis', title='', y_label_hist='Histogram (fraction)', histogram_log=True, histogram_fraction=True,
                 yscale='linear', ylim=None, plot_background_distribution=True):
        self.models_data = list()
        self.e_bins = bins
        self.plot_background_distribution = plot_background_distribution
    ...
    def draw(self, name_tag_formatter=None, return_fig=True, ax=None, type_='step', equal_spacing=False, swap_axis=None,
             swap_colors=('tab:blue', 'tab:orange'), **kwargs):
        """
        ...
        if type(self.plot_background_distribution) is bool:
            plot_background_distribution=self.plot_background_distribution
        else:
            plot_background_distribution=True
        ...
        if swap_axis:
            ax2 = ax1.twinx()
            assert self.plot_background_distribution==False
        ...
        for model_data in self.models_data:
            ...
            if plot_background_distribution:
                ...
                if type(self.plot_background_distribution) is bool:
                    ...
                    print("X", model_data)
                    make_plot = model_data['tags'][self.plot_background_distribution]
            ...
            if type_=='step':
                assert equal_spacing==False
                if error_exists:
                    err_1 = ((np.array(mean) + error/2)).tolist()
                    err_2 = ((np.array(mean) - error/2)).tolist()
                    ax1.step(e_bins, [mean[0]] + mean, label=name_of_plot, **kwargs)
                ...
                for k,v in kwargs.items():
                    if callable(v):
                        ...
                        kwargs2[k] = v
                if 'legend_loc' in kwargs2:
                    kwargs2.pop('legend_loc')

                if equal_spacing:
                    e_bins = np.linspace(0,1, len(e_bins))

                ax1.step(e_bins, [hist_values[0]] + hist_values, color='tab:gray', alpha=0)

                xbinwidth = (e_bins[1:] - e_bins[0:-1])/2 if not equal_spacing else 0.0
                # print(kwargs2)
                if swap_axis is None:
                    if error_exists:
                        ax1.errorbar((e_bins[0:-1] + e_bins[1:])/2, mean,yerr=error, xerr=xbinwidth, label=name_of_plot,**kwargs2)
                    else:
                        ax1.errorbar((e_bins[0:-1] + e_bins[1:])/2, mean,yerr=e_bins[1:]*0.0, xerr=xbinwidth, label=name_of_plot,**kwargs2)
                else:
                    ax1.tick_params(axis='y', colors=swap_colors[0])
                    ax2.tick_params(axis='y', colors=swap_colors[1])
                    for i in range(len(e_bins)-1):
                        if not swap_axis(mean[i]):
                            draw_on_ax = ax1
                            draw_with_color=swap_colors[0]
                        else:
                            draw_on_ax = ax2
                            draw_with_color=swap_colors[1]
                        draw_on_ax.errorbar([(e_bins[i] + e_bins[i+1])/2], [mean[i]], color=draw_with_color, yerr=[error[i]], xerr=xbinwidth, label=name_of_plot,**kwargs2)
            elif type_=='hlines':
                if 'fmt' not in kwargs:
                    kwargs['fmt'] = 'o'
                if 'elinewidth' not in kwargs:
                ...
                if 'legend_loc' in kwargs2:
                    kwargs2.pop('legend_loc')
                ax1.step(e_bins, [hist_values[0]] + hist_values, color='tab:gray', alpha=0)
                # ax1.errorbar(mean, e_bins[0:-1], e_bins[1:],yerr=error, label=name_of_plot,**kwargs2)
                # ax1.step(e_bins, [mean[0]] + mean, label=name_of_plot, color='tab:grey')
        ...
        if do_legend:
            ax1.legend(loc='center right' if 'legend_loc' not in kwargs else kwargs['legend_loc'])
        ...
        if self.ylim is not None:
            ...
            min_y_existing, max_y_existing = ax1.get_ylim()
            min_y = min_y if min_y is not None else min_y_existing
            max_y = max_y if max_y is not None else max_y_existing
            ax1.set_ylim(min_y, max_y)


        if return_fig and ax==None:
            return fig
